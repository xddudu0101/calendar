var CalUtils = {
	...
	init : function(calendar) {
		CalUtils.cid = calendar.cid;
		CalUtils.setMonthAndDay(calendar.today);
		CalUtils.draw(calendar.signList, calendar.signImg);
	},
	// 获取当前选择的年月
	...
	draw : function(signList, signImg) {
		...
		var str = CalUtils.drawCal(CalUtils.showYear, CalUtils.showMonth,
				signList, signImg);
		$("#" + CalUtils.cid).html(str);
		// 绑定日历表头
		var calendarName = CalUtils.showYear + "年" + CalUtils.showMonth + "月";
	...
	// 获取当前月的周数（日历行数）
	getWeeks : function() {
		var y = CalUtils.showYear;
		var m = CalUtils.showMonth;
		var today = new Date();
